...
**第三方 Node-RED 节点装不了。** 面板里的「节点管理」要连 npmjs.org，
离线现场没有这条路。包里已经带上 `@thinglinks` 自有节点（设备注册、点位上报、上行出口），
它们随镜像发布、不走 npm。

要用第三方节点有两条路，都需要提前准备：

1. **在有网的同架构机器上**装好节点，把实例数据目录整个拷到现场
2. 现场自建 npm 私有源（verdaccio 之类），并在实例的 `settings.js` 里指向它
