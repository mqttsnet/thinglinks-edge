...
> **32비트 ARM 은 배포하지 않습니다.** 결정적인 이유는 하류에 있습니다 —— Node-RED
> 공식 이미지의 5.x 계열은 `amd64` / `arm64` 뿐이라, `armv7` 에서 Manager 가 뜨더라도
> 인스턴스는 Node-RED 4.1.x 에 영구히 묶입니다. 빌드 측 비용도 더해집니다
> (`better-sqlite3` 에 32비트 ARM 사전 빌드 바이너리가 없음).
> 애초에 적합하지도 않습니다: 진짜 32비트 전용 SoC(i.MX6, AM335x, A20)는
> 보통 256MB~1GB 메모리뿐인데, Manager 는 실측 53 MiB, 인스턴스마다 약 104 MiB 가 필요합니다.
>
> **Raspberry Pi 사용자는 대부분 해당되지 않습니다**: Pi 3 / 3B+ / Zero 2 W 는 모두
> 64비트를 지원하며, 32비트 OS 를 설치한 경우에만 `armv7` 로 분류됩니다.
